...
use spin_sdk::{http_component, sqlite::Connection as SqliteConnection, variables};
use std::sync::Arc;
...
async fn handle_lexodus(req: IncomingRequest, resp_out: ResponseOutparam) {
    ...
    let sqlite_connection =
        Arc::new(SqliteConnection::open("default").expect("Failed to open benwis_leptos db"));
    // let con =
    //     Arc::new(spin_sdk::sqlite::Connection::open("default").expect("Failed to open lexodus db"));
    let postgres_connection =
        Arc::new(pg::Connection::open(&postgres_db).expect("Failed to open postgres db"));
